...
//this is  in  memory db
  ...
  beforeEach(async () => {
    ...
    const f1={firstname:"Peter",lastname:"Pan",email:"pp@b.dk",password:hashedPW}
    const f2={firstname:"Donald",lastname:"Duck",email:"dd@b.dk",password:hashedPW}
    const f3={firstname:"Peter",lastname:"Admin",email:"peter@admin.dk",password:hashedPW}
    const status=friendsCollection.insertMany([f1,f2,f3])
  ...
  describe("Verify the findNearbyFriends method", () => {
    it("Should Not find xxxxxxxx@b.dk", async () => {
      await expect(positionFacade.findNearbyFriends("xxxxxxxx@b.dk", 12.48, 55.77, DIST_TO_SEARCH)).to.be.rejectedWith(ApiError)
    })
  })
